...
object AuthentikConfig {
    ...
    const val ISSUER_URL = "https://auth.ctslab.net/application/o/ptalk-signature/"
    ...
    const val END_SESSION_ENDPOINT = "https://auth.ctslab.net/application/o/ptalk-signature/end-session/"

    // Client credentials (from Authentik blueprint)
    const val CLIENT_ID = "ptalk-signature-client"
    const val CLIENT_SECRET = "ptalk-signature-secret-key"

    // Redirect URI (must match Authentik provider config)
    const val REDIRECT_URI = "app://ptalksignature/callback"
